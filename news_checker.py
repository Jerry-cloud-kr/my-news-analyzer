...
try:
    ...
    if not OPENAI_API_KEY_FROM_SECRETS:
         ...
         st.stop()
    openai.api_key = OPENAI_API_KEY_FROM_SECRETS
except KeyError:
    # 로컬 테스트 시 st.secrets["OPENAI_API_KEY"]가 없을 때
    # 여기에 직접 키를 넣거나, 아래 st.error를 활성화하여 안내할 수 있습니다.
    # 주의: 아래에 직접 키를 입력하는 경우, 절대로 GitHub에 그대로 올리면 안 됩니다!
    OPENAI_API_KEY_LOCAL = "sk-proj-..." # << 로컬 테스트용 OpenAI 키 (실제 키로 대체하고, GitHub 푸시 전 삭제 또는 Secrets 방식으로 변경)
    if OPENAI_API_KEY_LOCAL == "sk-proj-...": # 플레이스홀더인지 확인
        st.error("OpenAI API 키를 Streamlit Secrets에서 찾을 수 없습니다. 로컬 테스트를 원하시면 코드에 임시 키를 직접 입력하거나, 앱 배포 후 Streamlit Community Cloud의 Secrets 설정을 확인하세요.")
        st.stop()
    ...
    st.error(f"OpenAI API 키 설정 중 오류: {e}")
    st.stop()
...
try:
    ...
    GOOGLE_AI_API_KEY_LOCAL = "AIzaSyD..." # << 로컬 테스트용 Gemini 키 (실제 키로 대체하고, GitHub 푸시 전 삭제 또는 Secrets 방식으로 변경)
    if GOOGLE_AI_API_KEY_LOCAL == "AIzaSyD...": # 플레이스홀더인지 확인
        st.error("Google AI API 키를 Streamlit Secrets에서 찾을 수 없습니다. 로컬 테스트를 원하시면 코드에 임시 키를 직접 입력하거나, 앱 배포 후 Streamlit Community Cloud의 Secrets 설정을 확인하세요.")
        st.stop()
    genai.configure(api_key=GOOGLE_AI_API_KEY_LOCAL)
...
def summarize_text(text):
    # 아래 import는 전역으로 이미 선언했으므로 함수 내에서는 필요 없습니다.
    # import google.generativeai as genai 
    model = genai.GenerativeModel(
        model_name='gemini-1.5-flash-latest',
        system_instruction="너는 뉴스 기사의 핵심 내용을 객관적으로 요약하는 AI야."
    )
    # ... (나머지 summarize_text 함수 코드는 그대로) ...
    prompt = f"""
    ...
    try:
        ...
        print(f"요약 API 오류: {e}") # 개발자 확인용 콘솔 출력
